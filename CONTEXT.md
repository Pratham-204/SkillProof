...
**Declared-Only**:
An Evidence Card state (`evidence_type = "declared_only"`) for a Skill Tag matched only via its Detection Pattern in a manifest, with zero Volume — the dependency is listed but no commit was ever found touching it. Originally produced a small nonzero Confidence Score from Presence alone rather than zero; since the Span Ceiling (round 11, ADR-0013), that Presence-only score is itself zeroed — a `declared_only` card always has `span_days = 0` (no qualifying Depth/Span evidence exists for a manifest-only match), so the Ceiling's multiplier is 0. This isn't unique to `declared_only`: any card with `span_days = 0` scores 0 regardless of `evidence_type` — see the round 11 note below. `declared_only` is still kept distinct from `none` (and from a zero-scoring `verified` card) as a named `evidence_type` (a Recruiter or `/search` result can still tell a bare listing apart from no evidence, or from real-but-Depth/Span-unqualified commits), but none of these states are distinguishable by Confidence Score alone anymore.
...
- The Ceiling applies uniformly to the final Confidence Score, keyed only on `span_days` — with no carve-out for any particular `evidence_type`. `span_days = 0` (no item cleared the Depth qualifying floor) zeroes the score regardless of how strong Presence/Volume/Depth were otherwise. This most visibly affects `declared_only` (see the Declared-Only term above), which *always* has `span_days = 0` and so is *always* zeroed — narrowing round 6's "declared-only produces a small nonzero score, not zero" guarantee the same way ADR-0004 itself narrowed the round-2 "zero qualifying evidence → 0" guarantee before it. But it isn't declared_only-specific: a `verified` card can hit the same `span_days = 0` floor too (real Volume-qualifying commits whose text never clears the Depth similarity floor) and gets zeroed identically. `evidence_type` (not Confidence Score) is now what distinguishes any of these zero-scoring states from each other or from `none`.
