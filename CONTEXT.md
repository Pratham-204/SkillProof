...
- Private repos now count as evidence, reversing round 3's "read-only public scope" for the Candidate's own repos specifically. `github_oauth_scope` now requests `repo` in addition to `read:user` — GitHub's classic OAuth scopes have no read-only-private-repos option, so this is also the only scope that grants write access the app never uses. `GitHubClient.list_owned_repos` (renamed from `list_owned_public_repos`) calls the authenticated `/user/repos` endpoint instead of the public-profile `/users/{login}/repos` one, falling back to the old endpoint (public-only) when the authenticated call fails — which is exactly what happens for a Candidate whose stored token predates this change and hasn't reconnected. No backfill/migration: that Candidate's next `/verify` is public-only exactly as before, until they reconnect.
- Scope is deliberately narrower than "everything the token can see": only repos the Candidate *owns* gain private visibility. External (non-owned) repos — reached via `list_merged_prs`, per ADR-0004's existing Volume scoping — are untouched by this round and stay public-only, since a private repo the Candidate merely contributed to belongs to someone else, whose consent this app has no way to obtain.
- An Evidence Card is still a public, unauthenticated URL (Evidence Card term, above) — private-repo evidence counts toward the Confidence Score exactly like public evidence, but `source_commits` redacts which repo backed it (`repo` becomes the literal string "a private repository", `url` becomes empty) rather than either exposing the repo name/link to anyone holding the card's URL, or hiding the evidence's existence entirely. Each redacted item still carries `private: true` so the UI can render "commit in a private repo" instead of a broken/empty link. See ADR-0016.
