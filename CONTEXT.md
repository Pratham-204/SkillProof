...
**Declared-Only**:
An Evidence Card state (`evidence_type = "declared_only"`) for a Skill Tag matched only via its Detection Pattern in a manifest, with zero Volume — the dependency is listed but no commit was ever found touching it. Originally produced a small nonzero Confidence Score from Presence alone rather than zero; since the Span Ceiling (round 11, ADR-0013), that Presence-only score is itself zeroed — a `declared_only` card always has *zero qualifying (Depth/Span) evidence*, not just `span_days = 0`, so the Ceiling's floor (which only applies when real qualifying evidence exists at all) doesn't rescue it and the multiplier stays exactly 0. A `verified` card can hit the same zero-qualifying-evidence case too (real Volume-qualifying commits whose text never clears the Depth similarity floor) and is likewise zeroed with no floor — but a `verified` card with at least one Depth-qualifying commit, even a single one on one day, gets the floor instead and stays nonzero. `declared_only` is still kept distinct from `none` and from a zero-scoring `verified` card as a named `evidence_type` (a Recruiter or `/search` result can still tell a bare listing apart from no evidence, or from real-but-Depth/Span-unqualified commits).
...
**Span Ceiling**:
A multiplicative cap on the final Confidence Score, applied on top of — not instead of — Span's existing weighted contribution. Uses its own smooth saturation curve and a saturation constant larger than Span's own, so a short burst of otherwise-strong evidence can't reach a high Confidence Score the way sustained evidence over time can. Not a fifth Signal; it sits outside the four-Signal weighted sum. Floored (not zeroed) at a small nonzero multiplier whenever real qualifying evidence exists at all — even a single commit, which has a mathematically degenerate `span_days = 0` — so genuine evidence isn't crushed to a literal 0.0 in an Evidence Card; a card with zero qualifying evidence at all still hits exactly 0, unaffected by the floor. See ADR-0013.
...
The single Candidate Dashboard button that covers both re-authenticating a Candidate whose stored token was revoked and authorizing as a different GitHub identity — both are the exact same `/auth/github/login` redirect. The outcome is decided entirely by which GitHub account authorizes on GitHub's side, not by any client-side toggle: the same `github_user_id` resumes the current Candidate with a refreshed token; a different one creates or resumes a wholly separate Candidate and silently replaces the active session, with no merge between the two Candidates' Evidence Cards. See round 12.
_Avoid_: Reconnect, switch account, sign in as someone else (all one button/action, not separate features)
...
- The Ceiling applies uniformly to the final Confidence Score with no carve-out for any particular `evidence_type` — but it's keyed on whether qualifying evidence exists at all, not on `span_days` alone. Zero qualifying evidence (no item ever cleared the Depth similarity floor) zeroes the score outright regardless of how strong Presence/Volume were otherwise. This most visibly affects `declared_only` (see the Declared-Only term above), which *always* has zero qualifying evidence and so is *always* zeroed — narrowing round 6's "declared-only produces a small nonzero score, not zero" guarantee the same way ADR-0004 itself narrowed the round-2 "zero qualifying evidence → 0" guarantee before it. A `verified` card can hit the same zero-qualifying-evidence case too and gets zeroed identically. `evidence_type` (not Confidence Score) is what distinguishes these zero-scoring states from each other or from `none`.
- A real, live `/verify` run surfaced that a single Depth-qualifying commit — the common case for a normal-sized repo — also produces `span_days = 0` (one date, so `max == min`), which is mathematically identical to the true zero-qualifying-evidence case above but not the same situation: it's one real, similarity-cleared piece of evidence with a degenerate span, not an absence of evidence. Applying the raw `n/(n+SPAN_CEILING_SATURATION_DAYS)` formula there crushed genuine, decent-similarity evidence to a literal 0.0 — far harsher than the "discounted, not eliminated" intent ADR-0013 describes. `SPAN_CEILING_FLOOR` (0.1) now floors the multiplier instead of letting it reach 0, but only when qualifying evidence exists at all — the true zero-qualifying-evidence floor case above is unaffected and still hits exactly 0.
...
- Reconnect and "use a different GitHub account" turn out to be the same mechanism — both are just a fresh `/auth/github/login` redirect, with the outcome decided entirely by which GitHub identity authorizes on GitHub's side (see Connect GitHub Account term above). No new backend logic was needed for account-switching itself: `callback()`'s existing lookup-or-create-by-`github_user_id` already does the right thing.
- Round 10 left sign-out explicitly out of scope; that stands — there's still no visible "sign out" control anywhere. Switching to a different GitHub account is implicit: authorizing as a different identity silently replaces the active session, with no separate log-out step or confirmation shown to the Candidate.
- The Connect GitHub Account button lives only on the Candidate Dashboard, not in the global `AppHeader` or on ClaimSkills/ScanReveal — keeping `AppHeader` session-blind (round 10's invariant) and avoiding the "broader account-settings surface" the Candidate Dashboard term's `_Avoid_` line already rules out. ClaimSkills and ScanReveal keep their existing conditional amber reconnect banners unchanged; the Dashboard's own amber banner is merged into the new button (same control, banner-style copy added when `needs_reconnect` is true) rather than shown alongside it.
- GitHub's OAuth authorize endpoint has no way to force an account picker (unlike e.g. Google's `prompt=select_account`) — if the browser's github.com session is still the same account, clicking Connect GitHub Account silently re-authorizes that same account with no error. The Dashboard button's copy calls this out rather than leaving a silent no-op unexplained.
- `/auth/github/callback` now deletes whatever `CandidateSession` row the incoming request's cookie pointed at (if any) before creating and setting the new one — fixing a pre-existing leak where every login/reconnect left the previous session row orphaned forever (no session GC exists otherwise). Broader TTL-based cleanup of all sessions stays out of scope; only the row displaced by this exact request is deleted.
